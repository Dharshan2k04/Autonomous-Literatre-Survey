...
resource "aws_security_group" "backend" {
  ...
  tags = merge(local.common_tags, { Name = "${local.name_prefix}-sg-backend" })
}
...
resource "aws_security_group" "frontend" {
  ...
  tags = merge(local.common_tags, { Name = "${local.name_prefix}-sg-frontend" })
}

resource "aws_security_group" "rds" {
  name        = "${local.name_prefix}-sg-rds"
  description = "RDS PostgreSQL: allow from backend tasks"
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "redis" {
  ...
  description = "ElastiCache Redis: allow from backend tasks"
  vpc_id      = aws_vpc.main.id
